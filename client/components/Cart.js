...
const Cart =() => {
  ...
  const h1 = {
    ...
    return (
      <div style={{position: 'relative', top: '10em'}} className='cart-container'>
            ...
            <tbody style={tableBody}>
              {localProducts.map(product =>
                <tr key={product.id}>
                  ...
                  <td style={td}><button onClick={()=>{ console.log(subtractQuantity(product.id)) }}>- </button>
                  <span id='quantity'>  1  </span>
                  <button onClick={()=>{ console.log(addQuantity(product.id)) }}> +</button></td>
                  <td style={td}><button onClick={()=>{removeProduct(product.id, localProducts)}}>Remove</button></td>
              </tr>
              )}
            </tbody>
          </table>
        ...
        <div style={buttonAlign}>
          <button  onClick={()=>{ addProduct(productsSample[0].id, productsSample) }} className="block mt-5 p-6 py-3 w-25 text-center rounded-full text-base font-bold bg-forest-green text-beige uppercase"
          >Product A</button>
          <button  onClick={()=>{ addProduct(productsSample[1].id, productsSample) }} className="block mt-5 p-6 py-3 w-25 text-center rounded-full text-base font-bold bg-forest-green text-beige uppercase"
          >Product B</button>
          <button  onClick={()=>{ addProduct(productsSample[2].id, productsSample) }} className="block mt-5 p-6 py-3 w-25 text-center rounded-full text-base font-bold bg-forest-green text-beige uppercase"
          >Product C</button>
        </div>
      </div>
    );
