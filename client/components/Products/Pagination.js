...
{/* Step Three: iterate over pageNumber array to create range of buttons to click on & set page number */}
          ...
          {pageNumbers.map((page, idx) => 
            ...
            currentPage === page
              ? <li key={idx}>
                <button id="pageButton" onClick={()=>{ setCurrentPage(page), document.documentElement.scrollTop = 0 }} className="mt-6 mr-4 px-3 py-2 border-2 border-forest-green rounded-full text-base font-bold bg-forest-green text-beige"
                >
                  {page}
                </button>      
                </li>
              : <li key={idx}>
                <button id="pageButton" onClick={()=>{ setCurrentPage(page), document.documentElement.scrollTop = 0 }} className="mt-6 mr-4 px-3 py-2 border-2 border-forest-green rounded-full text-base font-bold bg-beige text-forest-green"
                >
                  {page}
                </button>      
                </li>
